...
void heap_insert(node **arr, int *size, node* new_element);
...
int main(){
    ...
    for(i = 0; i < num_char; i++){
        ...
        min_heap[i] = new_element;
    }
    scanf("%d", &total_bits);

    //calculate origin bits
    origin_bit = ceil(log2(num_char));
    total_bits *= origin_bit;

    build_heap(min_heap, size);
